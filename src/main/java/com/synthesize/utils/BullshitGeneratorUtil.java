...
public class BullshitGeneratorUtil {
    ...
    /* 主题 */
    public static String motif = "666";

    /* 需要的文章长度 */
    public static int content_length = 6000;
    ...
    /* 名人名言 */
    public static String[] famous_quotation = {
    ...
    public static String random_sentence(String[] list){
        int index = (int) Math.floor( Math.random() * list.length );
        return list[index];
    }

    public static int random_num(Integer min, Integer max){
        if(min == null){
            min = 0;
        }
        if(max == null){
    ...
    public static String random_famous_quotation(){
        String quotation = random_sentence(famous_quotation);
        quotation = quotation.replace("曾经说过", random_sentence(front) );
        quotation = quotation.replace("这不禁令我深思", random_sentence(back) );
        return quotation;
    }

    public static String random_discuss(){
        String s = random_sentence(discuss);
//      句子 = 句子.replace(RegExp("主题", "g"),主题);
        s = s.replace("主题", motif);
        return s;
    }

    public static String append_paragraph(String section){
        if(section.endsWith(" ")){
...
//        }
    ...
    public static String genArticle(String motif){
        return genArticle(motif,6000);
    }

    public static String genArticle(String motif, Integer content_length){
        BullshitGeneratorUtil.motif = motif;
        BullshitGeneratorUtil.content_length = content_length;
        List<String> content = new ArrayList<>();
//      for(String 空 : 主题){
        String section = "";
        Integer section_length = 0;
        while( section_length < content_length ){
            Integer num = random_num(null,null);
            if(num < 5 && section.length() > 200){
                section = append_paragraph(section);
                content.add(section);
                section = "";
            }else if(num < 20){
                String s = random_famous_quotation();
                section_length = section_length + s.length();
                section = section + s;
            }else{
                String s = random_discuss();
                section_length = section_length + s.length();
                section = section + s;
            }
        }
...
//      }
    ...
    public static void main(String[] args) {
        System.out.println(genArticle("666",1000));
    }
}
